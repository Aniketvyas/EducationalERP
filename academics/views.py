...
from .models import *
from outpass.models import stud_outing
import random
...
from stu.settings import EMAIL_HOST_USER 
...
def home(request):
    ...
    elif employee.objects.filter(EmployeeID=request.user):
...
def notice_index(request):
    return render(request,'registrar/registrar_notice_dash.html')

def notice(request):
    return redirect('/notice')

def NoticeRequest(request):
    Etype=employee.objects.filter(EmployeeID=request.user)
    for i in Etype:
        Etype = i.Etype
    if Etype == "registrar":
        notices = notice.objects.filter(status="pending")
        return render(request,'registrar/requests.html',{'notices':notices})
...
def Accept(request,id):
    ...
    for i in Etype:
        Etype = i.Etype
    if Etype == "registrar":
        notice.objects.filter(status="pending", noticeId = id).update(status="published")
        return redirect('/registrar/NoticeRequest')
...
def Decline(request,id):
    ...
    for i in Etype:
        Etype = i.Etype
    if Etype == "registrar":
        notice.objects.filter(status="pending", noticeId = id).update(status="declined")
        return redirect('/registrar/NoticeRequest')
...
def test(request):
    ...
    if request.method=="POST":
        ...
        print(courses)
        for i in courses:
            if name in( i.FieldOfExpertise or i.FieldOfInterest1 or i.FieldOfInterest2 or i.FieldOfInterest3):
                print("match Found")
                l.append(i.FacultyID)
            else:
                print('Not found')
                continue
        print(l)
...
def department(request,id):
    if id == "info":
        id = employee.objects.filter(EmployeeID=request.user.username)
        for i in id:
            id = i.Etype
        print(id)
        if id == 'DEANSOE':
            course = Department.objects.filter(School="ENGINEERING")
        elif id == 'DEANSOM':
            course = Department.object.filter(School="MANAGEMENT")
        else:
            course = 'none'
        print(course)
        return render(request,'dean/department.html',{'courses':course})
    if Department.objects.filter(DepartmentID=id).exists():
...
def asstlecture(request):
    obj = lectures.objects.filter(TaughtBy=faculty.objects.get(FacultyID = request.user))
    return render(request,'professors/lecture.html',{'lect':obj,'lecture':True})

def assLectureView(request,lecture):
    lectur = lectureEnrollment.objects.filter(lectureId=lectures.objects.get(pk=lecture))
    lects = lectures.objects.filter(pk=lecture)
    print("hello",lectur)
    for i in lectur:
        print(i.studentID.FirstName)
...
# def ProfLectures(request):
#     obj = lectures.objects.filter(TaughtBy=faculty.objects.get(FacultyID = request.user))
#     return render(request,'professors/lecture.html',{'lect':obj})
...
def trackLecture(request,id):
    if request.method == "POST":
        ...
        if lectureRecord.objects.filter(startDate=startDate,endDate=endDate):
            ...
            facultyId = faculty.objects.get(FacultyID=request.user)).save()
        ...
        obj = lectureRecord.objects.filter(facultyId=faculty.objects.get(FacultyID=request.user),lectureId=lectures.objects.get(pk=id))
...
def LectureEnrollments(request,id):
    l = stud_details.objects.filter(Department = Department.objects.get(HeadOfDepartment=request.user)).exclude(id__in = Subquery(lectureEnrollment.objects.filter(lectureId=lectures.objects.get(LectureID=id)).values('studentID')))
    context = {
        'student' : l,
        'lectureEnroll':True
    }
...
def LectureEnrollmentDone(request,LectureID,EnrollmentNumber):
    ...
    lect = lectures.objects.get(LectureID = LectureID)
    lect.attendies.add(data)
    print(lect.attendies)
    return redirect('/academic/HOD/lecture/'+str(LectureID)+'/Enrollment')
def lectureFilter(request , id):
    if request.method == "POST":
        sem = int(request.POST['sem'])
        students = stud_details.objects.filter(CurrSem = sem , Department = Department.objects.get(HeadOfDepartment = request.user))
        print(students)
        return render(request, 'hod/lecture.html',{'students':students})


def lol(request,lecture):
    lectur = lectureEnrollment.objects.filter(lectureId=lectures.objects.get(LectureID=lecture))
    lects = lectures.objects.filter(LectureID=lecture)
    print("hello",lectur)
    for i in lectur:
        print(i.studentID.FirstName)
...
def trackLectureHod(request,id):
    obj = lectureRecord.objects.filter(lectureId=lectures.objects.get(LectureID=id))
